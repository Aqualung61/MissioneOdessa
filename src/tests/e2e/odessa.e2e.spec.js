...
test('API /api/lingue restituisce array', async ({ request }) => {
  const response = await request.get('http://localhost:3001/api/lingue');
  expect(response.status()).toBe(200);
  const data = await response.json();
...
test('API /api/luoghi restituisce array', async ({ request }) => {
  const response = await request.get('http://localhost:3001/api/luoghi');
  expect(response.status()).toBe(200);
  const data = await response.json();
  expect(Array.isArray(data)).toBe(true);
...
test('Frontend /web/odessa1.html viene servito', async ({ page }) => {
  const response = await page.goto('http://localhost:3001/web/odessa1.html');
  expect(response.status()).toBe(200);
