...
```mermaid
flowchart TD
  U[Browser] -->|GET /web/odessa_main.html| S[Express static + Helmet CSP\n(src/server.js)]
  S -->|HTML| U

  subgraph HTML[web/odessa_main.html]
    H1[<script> inline\n(basePath/redirect/file://, bootstrap, fetch)]
    H2[<style> inline\n(regole pagina)]
    H3[style="..." in markup\n(wrapper, overlay, contatori, ecc.)]
    H4[<link rel=stylesheet>\ncss/base.css + css/components.css + css/odessa_main.css]
  ...
  subgraph JS[web/js/odessa1.js]
    J1[DOM updates]
    J2[usa element.style.*\n(color, fontWeight, cursor, display...)]
    J3[assegna handler JS\n(el.onclick = ...)]
  end
  H5 --> JS

  subgraph CSP[CSP effettiva\n(src/server.js)]
...
flowchart TD
  ...
  subgraph HTML[web/odessa_main.html]
    T1[SOLO riferimenti esterni\n(no <script> inline)]
    T2[SOLO CSS esterni\n(no <style> inline)]
    T3[SOLO classi/ID\n(no style="...")]
    T4[<link rel=stylesheet>\ncss/base.css + css/components.css + css/odessa_main.css]
  ...
  subgraph JS[web/js/odessa1.js (refactor)]
    R1[DOM updates]
    R2[usa classList.*\n(al posto di element.style.*)]
    R3[addEventListener\n(al posto di el.onclick=...)]
  end
  T6 --> JS
  ...
  subgraph CSP[CSP target\n(src/server.js)]
    D1[script-src: 'self']
    D2[script-src-attr: 'none' (se niente on* HTML)]
    D3[style-src: 'self' (se niente inline style)]
  end
  S --> CSP
```
...
sequenceDiagram
  ...
  Note over U,S: CSP “stretta” blocca (1) <script> inline, (2) onclick=..., (3) style="..." se non consentiti
```
