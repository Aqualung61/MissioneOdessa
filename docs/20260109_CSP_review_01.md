...
- Le raccomandazioni operative qui sotto sono focalizzate su:
  ...
  - `web/js/odessa1.js` (perché anche il JS può “forzare” l’uso di inline style)
...
- `index.html`
  ...
  - `onclick="..."` e vari `style="..."` (anche generati via template string in JS)
- `web/odessa_intro.html`
  - più `<script>...</script>` inline
  - `<style>...</style>` inline
  - alcuni `style="..."`
...
- Contiene **CSS inline** (`<style>...</style>`).
- Contiene numerosi **attributi `style="..."`** in markup.

#### `web/js/odessa1.js`
Non risultano usi di `eval()`/`new Function()` nel codice applicativo sotto `web/js`.
Tuttavia `web/js/odessa1.js` utilizza spesso `element.style.* = ...` per color/fontWeight/cursor/display ecc.
...
## Modifiche da realizzare (dettaglio) — `web/odessa_main.html` + `web/js/odessa1.js`
...
flowchart TD
  ...
  subgraph HTML[web/odessa_main.html]
    ...
    H2[style inline - regole pagina]
    H3[style attribute in markup - wrapper - overlay - contatori]
    H4[CSS esterni - base.css - components.css - odessa_main.css]
    H5[script src=js/odessa1.js type=module]
  end
...
flowchart TD
  ...
  subgraph HTML[web/odessa_main.html]
    ...
    T6[script src=js/odessa1.js type=module]
  end
  ...
  subgraph JS[web/js/odessa1.js refactor]
    R1[DOM updates]
...
sequenceDiagram
  ...
  participant G as js/odessa1.js module
...
| `script-src-attr` | Handler inline tipo `onclick="..."` | (oggi in scope: evitarli) | Se presenti, richiede `unsafe-inline` su attr | Usare `addEventListener` ovunque | `web/odessa_main.html`, `web/js/odessa1.js` |
...
| `style-src-attr` (opz.) | Attributi `style="..."` (browser moderni) | `style="..."` in markup; `element.style.*` in JS | Se impostata a `none`, rompe UI che usa style inline | Migrare a `classList.*` e classi CSS; poi valutare `style-src-attr 'none'` | `web/odessa_main.html`, `web/js/odessa1.js` |
...
| `connect-src` | Fetch/XHR/WebSocket | Fetch a `/api/...` | Se troppo stretta, rompe i fetch | Tenere `'self'` (ok) ed elencare eventuali domini extra solo se necessari | `web/js/i18n.js`, `web/js/odessa1.js` |
| `img-src` | Immagini (incl. `data:`) | immagini locali + eventuale `data:` | Se manca `data:`, può rompere placeholder/inline | Tenere `'self' data:` se serve | `web/odessa_main.html`, `web/js/odessa1.js` |
...
- (Opzionale ma consigliato se vuoi bloccare davvero gli style inline) Migrare in `web/js/odessa1.js` da `element.style.*` a `classList.*`.
...
- Se usi `/api/run-tests`: eseguire la suite smoke dopo ogni fase.
...
### 4) Ridurre/azzerare `element.style.*` in `web/js/odessa1.js` (solo se vuoi togliere `unsafe-inline` dagli style)
...
Output atteso:
- `web/js/odessa1.js` non imposta più stili inline, ma solo classi.
